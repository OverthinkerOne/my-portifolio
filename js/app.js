...
$(function() {
  ...
  // --------------------------------------------- //
  // Contact Form -> WhatsApp Redirect Start
  // --------------------------------------------- //
  (function() {
    var form = document.querySelector('form.contact-form');
    if (!form) return;

    var WHATSAPP_NUMBER = '5545999104170'; // DDI + DDD + número

    form.addEventListener('submit', function(e) {
      try {
        e.preventDefault();
        var nome = (form.querySelector('[name="nome"]').value || '').trim();
        var assunto = (form.querySelector('[name="assunto"]').value || '').trim();
        ...
        var texto = `Olá, Paulo! Vim pelo seu portfólio.\n` +
                    `Nome: ${nome}\n` +
                    (assunto ? `Assunto: ${assunto}\n` : '') +
                    `Mensagem: ${mensagem}`;

        var url = `https://api.whatsapp.com/send?phone=${WHATSAPP_NUMBER}&text=${encodeURIComponent(texto)}`;
        // Redireciona para o WhatsApp (abre o app no celular ou Web WhatsApp no desktop)
...
// --------------------------------------------- //
// Contact Form Helpers End (WhatsApp flow doesn't need success banners)
// --------------------------------------------- //
